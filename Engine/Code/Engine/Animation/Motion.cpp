	...
	float inTime)
{
	...
	else if (outFrameIndex0 == (uint32_t)(m_frameCount - 2))
	{
		...
		outBlend = ClampFloat(outBlend, 0, 1.f);
	}
	else
	{
...
void Motion::Update(Skeleton* skeleton, float deltaSeconds)
{
	...
	//Set m_currentTime based on the mode
	switch (m_wrapMode)
	{
		case (Motion_WrapMode::WRAPMODE_CLAMP) : //Do nothing when we are after max time
		{
		...
		case (WRAPMODE_LOOP) : // Start at beginning of animation when we are after max time
		{
			if (m_age > m_totalLengthSeconds)
			{
				m_age = 0.0f;
		...
		case (Motion_WrapMode::WRAPMODE_PINGPONG) :
		{
...
void Motion::WriteToFile(std::string fileName)
{
...
void Motion::LoadFromFile(std::string fileName)
{
