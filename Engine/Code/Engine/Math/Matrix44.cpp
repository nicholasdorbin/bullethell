...
void MatrixSetForward(mat44_fl const *mat, Vector3 const &forward)
{
	Vector4 row;
	MatrixGetRow(mat, 2, &row);
...
void mat44_fl::MakeRotationEuler(float yawDegrees, float pitchDegrees, float rollDegrees, Vector3 const &position)
{
	...
	// yaw = y
	// roll = z
	float sx = sin(DegToRad(pitchDegrees));
	float cx = cos(DegToRad(pitchDegrees));

	float sy = sin(DegToRad(yawDegrees));
	float cy = cos(DegToRad(yawDegrees));

	float sz = sin(DegToRad(rollDegrees));
	float cz = cos(DegToRad(rollDegrees));
	...
	*/
	...
	/*
	float sx = SinDegrees(pitchDegrees);
	float cx = CosDegrees(pitchDegrees);

	float sy = SinDegrees(yawDegrees);
	float cy = CosDegrees(yawDegrees);

	float sz = SinDegrees(rollDegrees);
	float cz = CosDegrees(rollDegrees);

	Vector3 const right(cy*cz + sx*sy*sz, -cx*sz, -cz*sy + cy*sx*sz);
	Vector3 const up(cy*sz - cz*sx*sy, cx*cz, -sy*sz - cy*cz*sx);
	Vector3 const forward(-cx*sy, -sx, -cx*cy);
	Vector3 const o(position.x, position.y, position.z);

	MatrixSetBasis(this, right, up, forward, o);
	*/
	//MakeBasis(right, up, forward);
	//SetTranslation(position.x, position.y, position.z);
}

void mat44_fl::MatrixForsethCOBNick()
{
	//temp = y
	float yCol0 = data[1];
	float yCol1 = data[5];
